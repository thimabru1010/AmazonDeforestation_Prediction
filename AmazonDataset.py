...
class IbamaDETER1km_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = cv2.dilate(data.astype(np.uint8), self.kernel, iterations=2).astype(np.float32)
        labels = cv2.dilate(labels.astype(np.uint8), self.kernel, iterations=2).astype(np.float32)
        labels[:, mask == 0] = -1          
