...
work_dirs_path = Path(f'/home/thiago/AmazonDeforestation_Prediction/OpenSTL/work_dirs')
...
for work_dir in work_dirs:
    ...
    channel_input = 0
    for i, channel_used in enumerate(exp_data):
        if channel_used:
            channel_input+= inputs_shape[i]
    ...
    custom_training_config = {
        ...
        'dataname': 'custom',
        'in_shape': [4, channel_input+1, 64, 64], # T, C, H, W = self.args.in_shape
        'loss_weights': None,
        'early_stop_epoch': 10,
    ...
    if data_prep == 'giov':
        test_data, patches_sample_train, patches_sample_val, frames_idx, county_data, counties_time_grid, \
            precip_time_grid, tpi_array, scores_time_grid, night_time_grid = prep4dataset_test(giov_config)
        ...
        # print('DEBUG')
        # print(exp_data)
        county_data = county_data if exp_data[0] else None
        precip_time_grid = precip_time_grid if exp_data[1] else None
        scores_time_grid = scores_time_grid if exp_data[2] else None
        night_time_grid = night_time_grid if exp_data[3] else None
